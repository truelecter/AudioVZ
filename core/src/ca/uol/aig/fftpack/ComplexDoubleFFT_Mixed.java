...
class ComplexDoubleFFT_Mixed {
    ...
    /*----------------------------------------------------------------------
     ...
     isign is +1 for backward and -1 for forward transforms
     ----------------------------------------------------------------------*/
    void passfg(int nac[], int ido, int ip, int l1, int idl1, final double cc[], double c1[], double c2[], double ch[],
        ...
        if (idot <= l1) {
            ...
            for (j = 1; j < ip; j++) {
                ...
                for (i = 3; i < ido; i += 2) {
                    idij += 2;
                    for (k = 0; k < l1; k++) {
                        c1[i - 1 + (k + j * l1) * ido] = wtable[idij - 2 + iw1] * ch[i - 1 + (k + j * l1) * ido]
                                - isign * wtable[idij - 1 + iw1] * ch[i + (k + j * l1) * ido];
                        c1[i + (k + j * l1) * ido] = wtable[idij - 2 + iw1] * ch[i + (k + j * l1) * ido] + isign
                                * wtable[idij - 1 + iw1] * ch[i - 1 + (k + j * l1) * ido];
                    }
                }
            }
        } else {
